...
ROM::SQL.migration do
  up do
    ...
    # Feature: email_auth
    create_table :user_email_auth_key do
    ...
    create_table :user_login_failure do
      foreign_key :id, :user, primary_key: true, type: :Bignum
      column :number, Integer, null: false, default: 1
    end

    # Feature: reset_password
    create_table :user_password_reset_key do
      foreign_key :id, :user, primary_key: true, type: :Bignum
    ...
    create_table :user_verification_key do
      foreign_key :id, :user, primary_key: true, type: :Bignum
    ...
    create_table :user_login_change_key do
      foreign_key :id, :user, primary_key: true, type: :Bignum
    ...
    run "GRANT SELECT, INSERT, UPDATE, DELETE ON user_email_auth_key TO CURRENT_USER"
    run "GRANT SELECT, INSERT, UPDATE, DELETE ON user_jwt_refresh_key TO CURRENT_USER"
    run "GRANT SELECT, INSERT, UPDATE, DELETE ON user_lockout TO CURRENT_USER"
    run "GRANT SELECT, INSERT, UPDATE, DELETE ON user_login_change_key TO CURRENT_USER"
    run "GRANT SELECT, INSERT, UPDATE, DELETE ON user_login_failure TO CURRENT_USER"
    run "GRANT SELECT, INSERT, UPDATE, DELETE ON user_password_reset_key TO CURRENT_USER"
    ...
    run "GRANT SELECT, INSERT, UPDATE, DELETE ON user_verification_key TO CURRENT_USER"
  end

  down do
    drop_table :user_login_change_key,
