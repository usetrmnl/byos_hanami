...
Extensions are similar to Plugins as found in our {core_link} application. With extensions, the goal is to encompass all plugin capabilities under a single umbrella. This also fits better with the architecture of this application and allows this application to evolve and expand upon what is possible.
...
To help you get used to how Terminus extensions work versus Core plugins, let's walk through a scenario building a Core plugin and Terminus extension side-by-side so you can see the differences. To get started, create a new link:https://trmnl.com/plugin_settings/new?keyname=private_plugin[private plugin] in Core and then do the same in Terminus but create a new Extension instead. Here's the steps:
...
You can attach sensors to your TRMNL-compatible device and Terminus will immediately capture this information for rendering in your Liquid templates.

⚠️ Support is implemented in Terminus be we need to add {firmware_link} support next. There is a feature branch in the works that, once folded in, will appear in a future version release.

Behind the scenes, the way this works is your device will include the `HTTP_SENSORS` header which adheres to {rfc_2626_link} when making a `/api/display` request with content similar to the following:
...
This allows each device to answer an empty array, a single record, or multiple records. All of this is saved in the database as sensor records for which you can access when building extensions. In the future a HTTP POST request will be used for `/api/display` since header length has limits.

=== Server

You can attach sensors to your Raspberry PI which allows you to access sensor records when building extensions. This is fully explained in the link:/doc/raspberry_pi.adoc[Raspberry Pi Documentation] so please follow the link for details.
