...
Rodauth::Feature.define :hanami do
  ...
  auth_value_method :templates_root, "authentication"
  ...
  def render name
    ...
    view_rendering.template view_template(name), view_rendering.scope(rodauth: self)
  end
  ...
  def view_template? name
    ...
    view_rendering.renderer.__send__ :lookup, view_template(name), view_base.config.default_format
    # rubocop:enable Style/SendWithLiteralMethodName
  end

  def view_template(name) = "#{templates_root}/#{name.tr "-", "_"}"
