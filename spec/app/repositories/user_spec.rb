...
RSpec.describe Terminus::Repositories::User, :db do
  ...
  describe "#find_by" do
    ...
    it "answers record when found by multiple attributes" do
      expect(repository.find_by(name: user.name, label: user.label)).to have_attributes(user.to_h)
    end
  ...
  describe "#search" do
    let(:user) { Factory[:user, label: "Test"] }
  ...
  describe "#where" do
    it "answers record for single attribute" do
      expect(repository.where(label: user.label)).to contain_exactly(user)
    end

    it "answers record for multiple attributes" do
      expect(repository.where(label: user.label, name: user.name)).to contain_exactly(user)
    end
