...
* Used {esbuild_link} for asset management.
...
⚠️ This script is not idempotent which means you can't run it more than once since your database credentials will be different each time. It's only meant for getting up and running quickly to explore the feature set, not permanent use. To customize, see the link:doc/docker.adoc[Docker] documentation.
...
* link:https://github.com/simplecov-ruby/simplecov[SimpleCov]: Generates interactive reports are for every CI build. Will fail the build if quality drops below 95%. ⚠️ The badge at the top of this document isn't updated in real-time, unfortunately, but is fairly accurate since this project is configured to strive for 100% code coverage.
