...
* Uses {redis_link} for the key/value cache.
...
. {redis_link} (optional, for development).
...
. {trmnl_link} device (can use a virtual device too).
...
* `REDIS_DATABASE`: Defines your database ID. Used by {docker_doc_link} only. Default: `0`.
* `REDIS_PASSWORD`: Defines your password. Used by {docker_doc_link} only. Default: (auto-generated for you during setup).
* `REDIS_PORT`: Defines your port. Used by {docker_doc_link} only. Default: `6379`.
