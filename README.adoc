...
* `APP_SECRET`: Used for session, cookie, and Cross-Site Request Forgery (CSRF) protection. This is automatically created for you during setup but can be customized as desired.
...
You'll want to customize the environment variables in `.env` so your device can connect to your Docker service. Here's a few examples:

[source,bash]
----
# With specific IP address and port.
HANAMI_PORT=2300
API_URI=http://192.168.1.1

# With hostname.
API_URI=https://terminus.demo.io
----

You'll also want ensure your credentials are secure as well.

=== Compose

Use {docker_compose_link} to quickly launch the entire stack for development or production environments. Example:
